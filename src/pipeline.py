...
from transforma_dados import (listar_raws, carregar_raw,
                        transformar, validar) 
...
def extract() -> None:   #E: coleta a cotacao atual e acrescenta um raw."""
    caminho = salvar_raw(coletar_cotacoes())
    logger.info("raw salvo em %s", caminho)

def transform() -> pd.DataFrame:  #T: reconstroi a foto inteira a partir de TODOS os raws."""
...
def load(df: pd.DataFrame) -> None: #L: grava a foto inteira. replace = reexecutavel sem medo."""
